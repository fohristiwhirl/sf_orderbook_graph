...
import copy, json, queue, requests, threading
...
def liststocks(venue, verbose = False):
    return get_json_from_url(_API_URL + "venues/{}/stocks".format(venue), verbose = verbose)

# The following 2 functions implement an "orderbook ticker" by spinning up threads to repeatedly ask for orderbooks...

def orderbook_requests_thread(venue, symbol, output_queue):
	while 1:
		r = orderbook(venue, symbol)
		if r:
			output_queue.put(r)
		# time.sleep(random.uniform(1,2))

def orderbook_pseudoticker(venue = None, symbol = None, threads = 5, output_queue = None):
	
	if venue is None or symbol is None or output_queue is None:
		print("Orderbook pseudoticker failed to start: need venue, symbol, and output queue")
		return
	
	for n in range(threads):
		newthread = threading.Thread(target = orderbook_requests_thread, daemon = True, kwargs = {
										"venue"			: venue,
										"symbol"		: symbol,
